...
function procOrder(o) {
  ...
  if (o.fulfillments?.length > 0) {
    ...
    else if (['in_transit','confirmed'].includes(ss)) ts = 'preluat'; // scanat de curier = preluat
    else if (ss === 'label_printed') ts = 'pending'; // etichetă printată = încă la expeditor
    else if (o.fulfillment_status === 'fulfilled') ts = 'pending'; // fulfilled fără scan = neexpediat
  }
...
export default function Dashboard() {
  ...
  const sA=sum(['incurs','outfor','preluat']), sR=sum(['retur','anulat']);
