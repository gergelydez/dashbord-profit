...
// Fix: convert ArrayBuffer to base64 without stack overflow
...
export default function ImportCalc() {
  ...
  // ── AI PDF parsing — FIXED: no stack overflow ──
  const analyzePDF = async (file, type) => {
    setAiLoading(type); setError('');
    try {
      const arrayBuffer = await file.arrayBuffer();
      // Fix: use chunked conversion instead of spread operator
      const base64 = arrayBufferToBase64(arrayBuffer);

      const systemPrompt = type === 'dvi'
        ? `Ești un expert în declarații vamale românești (DVI). Extrage din documentul PDF următoarele câmpuri și răspunde DOAR cu JSON valid, fără text înainte sau după:
{
...
IMPORTANT: TVA se calculează pe (valoare marfă + transport + taxă vamală + comision), nu doar pe valoarea mărfii.`
        : `Ești un expert în facturi DHL România. Extrage din documentul PDF și răspunde DOAR cu JSON valid:
{
  ...
  "dataFactura": "<DD/MM/YYYY>"
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [{
              ...
              type: 'text',
              text: type==='dvi'
                ? 'Extrage datele din această Declarație Vamală de Import și returnează DOAR JSON.'
                : 'Extrage datele din această factură DHL și returnează DOAR JSON.'
            }]
          }]
        })
      });
      ...
      if (!jsonMatch) throw new Error('Nu am putut extrage datele din PDF. Răspuns AI: ' + text.slice(0,200));
  ...
  // TVA = (valoare + transport + taxă vamală + comision) × TVA%
  // Conform legislației române: TVA se aplică pe valoarea în vamă + taxe vamale + comision
  ...
  const prods = products.map(p=>{
    ...
    // TVA pe: valoare + transport + taxă vamală + comision (conform DVI)
  ...
  const exportJSON = () => {
    const data = {
      meta:{
        ...
        totalCostRON,
        notaTVA: 'TVA calculat pe: valoare marfă + transport + taxă vamală + comision DHL',
      ...
      produse: prods.map(p=>({
        ...
        taxeAlocateRON: +(p.costuri/Math.max(p.qty,1)).toFixed(2),
  ...
  const exportCSV = () => {
    ...
    prods.forEach(p=>rows.push([p.sku,`"${p.name}"`,p.qty,fmt(p.unitUSD),fmt(p.unitUSD*curs),fmt(p.costuri/Math.max(p.qty,1)),fmt(p.costUnit),fmt(p.totalP)]));
  ...
  const stepBtn=(s)=>({
    ...
    borderRadius:22,padding:'6px 18px',fontSize:11,cursor:'pointer',
      ...
      <div className="w">
        <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:28,paddingBottom:20,borderBottom:'1px solid #1a2535',flexWrap:'wrap'}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:'#64748b',marginTop:2}}>🤖 AI citește automat DVI + Factura DHL · Export JSON cu SKU pentru calculatorul de profit</div>
          </div>
                ...
                <div key={idx} className="prod-card">
                  ...
                  <div className="g2" style={{marginTop:8}}>
                    <div>
                      ...
                      <input type="number" style={inp} value={p.taxaVamala} step="0.1" min="0" placeholder={`default: ${taxaVamala||'0'}%`} onChange={e=>upd(idx,'taxaVamala',e.target.value)}/>
                      {p.taxaVamala==='0'&&<div style={{fontSize:10,color:'#10b981',marginTop:3}}>✓ Fără taxă vamală</div>}
                    </div>
                    <div>
                      <label style={lbl}>TVA %</label>
                      <input type="number" style={inp} value={p.tvaPercent} step="0.1" min="0" placeholder="21" onChange={e=>upd(idx,'tvaPercent',e.target.value)}/>
                    </div>
                  </div>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:10}}>
                    ...
                    {products.length>1&&<button onClick={()=>setProducts(p=>p.filter((_,i)=>i!==idx))} style={{background:'rgba(244,63,94,.1)',border:'1px solid rgba(244,63,94,.2)',color:'#f43f5e',borderRadius:7,padding:'3px 9px',cursor:'pointer',fontSize:11}}>✕</button>}
                  </div>
                </div>
              ))}
              <button onClick={()=>setProducts(p=>[...p,{...EMPTY}])} style={{background:'transparent',border:'1px dashed #1a2535',color:'#475569',padding:10,borderRadius:9,cursor:'pointer',fontSize:12,width:'100%',marginTop:4}}>+ Adaugă produs manual</button>
              {totalUSD_val>0&&(
            ...
            <div style={sec}>
              ...
              <div className="g2">
                <div>
                  ...
                  <input type="number" style={inp} value={transportUSD} step="0.01" placeholder="ex: 257.27" onChange={e=>{setTransportUSD(e.target.value);setTransportRON('');}}/>
                  <div style={{fontSize:10,color:'#475569',margin:'6px 0 4px'}}>— sau direct în RON —</div>
                  <input type="number" style={inp} value={transportRON} step="0.01" placeholder="ex: 1250.00 RON" onChange={e=>{setTransportRON(e.target.value);setTransportUSD('');}}/>
                </div>
                <div>
                  <label style={lbl}>Curs transport (dacă diferit de DVI)</label>
                  <input type="number" style={inp} value={cursTransport} step="0.0001" placeholder="ex: 4.3046" onChange={e=>setCursTransport(e.target.value)}/>
                  <div style={{fontSize:10,color:'#475569',marginTop:8}}>Lasă gol = se folosește cursul din DVI</div>
                  {transportUSD && cursT>0 && <div style={{marginTop:8,fontSize:11,color:'#10b981'}}>{fmtUSD(parseFloat(transportUSD))} × {cursT} = <strong>{fmtRON(parseFloat(transportUSD)*cursT)}</strong></div>}
                </div>
              </div>
            </div>
            <button className="nbtn" onClick={()=>setStep(2)} disabled={totalUSD_val===0}>Continuă → DVI & Taxe Vamale →</button>
          </div>
        )}
        ...
        {step===2&&(
          <div>
            ...
            <div style={{...sec,border:'1px solid rgba(168,85,247,.3)',background:'rgba(168,85,247,.04)'}}>
              ...
              {aiMessages.dvi&&<div style={{marginBottom:12,padding:'8px 12px',background:'rgba(16,185,129,.08)',border:'1px solid rgba(16,185,129,.2)',borderRadius:8,fontSize:11,color:'#10b981'}}>{aiMessages.dvi}</div>}
            ...
            <div style={{...sec,border:'1px solid rgba(59,130,246,.25)',background:'rgba(59,130,246,.03)'}}>
              ...
              {aiMessages.dhl&&<div style={{marginBottom:12,padding:'8px 12px',background:'rgba(16,185,129,.08)',border:'1px solid rgba(16,185,129,.2)',borderRadius:8,fontSize:11,color:'#10b981'}}>{aiMessages.dhl}</div>}
            ...
            <div style={sec}>
              ...
              {/* Nota TVA */}
              <div style={{marginBottom:14,padding:'8px 12px',background:'rgba(168,85,247,.06)',borderRadius:8,border:'1px solid rgba(168,85,247,.2)',fontSize:11,color:'#a855f7'}}>
                💡 <strong>TVA se calculează pe:</strong> valoare marfă + transport + taxă vamală + comision DHL (conform legislației române)
              </div>
              <div className="g2" style={{marginBottom:14}}>
                <div>
                  ...
                  {cursValutar&&<div style={{fontSize:10,color:'#10b981',marginTop:5}}>{fmtUSD(totalUSD_val)} × {cursValutar} = <strong>{fmtRON(totalRON_f)}</strong></div>}
                </div>
                <div>
                  <label style={lbl}>Taxă vamală (%)</label>
                  <input type="number" style={{...inp,borderColor:taxaVamala?'rgba(16,185,129,.4)':'#1a2535'}} value={taxaVamala} step="0.1" min="0" placeholder="ex: 3.7" onChange={e=>setTaxaVamala(e.target.value)}/>
                </div>
              </div>
              <div className="g2" style={{marginBottom:14}}>
                <div>
              ...
              <div style={{...sec,border:'1px solid rgba(249,115,22,.25)'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:2}}>
                  {[
                    ...
                    [`💰 TVA${tvaPercent?` ${tvaPercent}%`:''}${tvaRON_dvi?' (din DVI)':' (pe baza vamală)'}`,fmtRON(tva_RON_final),'#a855f7',false],
                ...
                <div style={{textAlign:'right',marginTop:8,fontSize:11,color:'#475569'}}>Cost mediu / buc: <strong style={{color:'#f97316'}}>{fmtRON(totalQty>0?totalCostRON/totalQty:0)}</strong></div>
              </div>
            )}

            <div style={{display:'flex',gap:10}}>
              <button className="bbtn" onClick={()=>setStep(1)}>← Înapoi</button>
              <button className="nbtn" style={{flex:2}} onClick={()=>setStep(3)} disabled={!cursValutar}>Calculează & Vezi Rezultatele →</button>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{...sec,border:'1px solid rgba(249,115,22,.35)',background:'linear-gradient(135deg,#0c1520 0%,#111d2b 100%)',marginBottom:18}} className="glow-orange">
              ...
              {[
                ...
                [`💰 TVA ${tvaPercent}% (pe baza vamală)`,tvaRON_dvi?'din DVI':'calculat',fmtRON(tva_RON_final),'#a855f7'],
              ].map(([l,sub,v,c])=>(
              ...
              <div style={{textAlign:'right',marginTop:6,fontSize:11,color:'#475569'}}>{totalQty} bucăți · cost mediu <strong style={{color:'#f97316'}}>{fmtRON(totalQty>0?totalCostRON/totalQty:0)}</strong> / buc</div>
            </div>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12,flexWrap:'wrap',gap:8}}>
                  <div>
                    ...
                    <div style={{display:'flex',gap:6,marginTop:4,flexWrap:'wrap'}}>
                      <span className="tag" style={{background:'rgba(59,130,246,.12)',color:'#3b82f6',border:'1px solid rgba(59,130,246,.25)'}}>SKU: {p.sku||'—'}</span>
                      <span className="tag" style={{background:'rgba(100,116,139,.1)',color:'#64748b',border:'1px solid #1a2535'}}>{p.qty} buc × ${fmt(p.unitUSD)}</span>
                      <span className="tag" style={{background:'rgba(249,115,22,.08)',color:'#f97316',border:'1px solid rgba(249,115,22,.2)'}}>{fmt(p.prop*100,1)}% din factură</span>
                    </div>
                  </div>
                  <button onClick={()=>setShowBD(showBD===idx?null:idx)} style={{background:'transparent',border:'1px solid #1a2535',color:'#64748b',borderRadius:8,padding:'4px 12px',cursor:'pointer',fontSize:11}}>{showBD===idx?'▲ ascunde':'▼ detalii'}</button>
                </div>
                <div className="g3">
                  {[
                    ...
                    {lbl:'COST UNITAR',sub:'cu TVA inclus',val:fmtRON(p.costUnit),col:'#f97316',bg:'rgba(249,115,22,.08)',bold:true},
                ...
                {showBD===idx&&(
                  <div style={{marginTop:12,background:'#070d12',borderRadius:9,padding:'12px 14px',border:'1px solid #1a2535'}}>
                    ...
                    {[
                      ...
                      [`TVA ${p.tvaPPerc||21}% (pe baza vamală)`,fmtRON(p.tvaProd||0),false],
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginTop:18}}>
              <button onClick={exportJSON} style={{background:'linear-gradient(135deg,#10b981,#059669)',color:'white',border:'none',padding:'14px',borderRadius:12,fontWeight:700,fontSize:13,cursor:'pointer',boxShadow:'0 4px 14px rgba(16,185,129,.3)'}}>💾 Export JSON cu SKU</button>
              <button onClick={exportCSV} style={{background:'linear-gradient(135deg,#3b82f6,#1d4ed8)',color:'white',border:'none',padding:'14px',borderRadius:12,fontWeight:700,fontSize:13,cursor:'pointer',boxShadow:'0 4px 14px rgba(59,130,246,.3)'}}>📊 Export CSV</button>
            </div>
            ...
            {savedResults&&(
              <div style={{marginTop:12,background:'rgba(16,185,129,.06)',border:'1px solid rgba(16,185,129,.2)',borderRadius:12,padding:'14px 16px'}} className="glow-green">
                ...
                <details>
                  ...
                  <pre style={{background:'#070d12',border:'1px solid #1a2535',borderRadius:8,padding:12,fontSize:9,color:'#64748b',overflowX:'auto',marginTop:8,maxHeight:280,overflow:'auto',lineHeight:1.6}}>{JSON.stringify(savedResults,null,2)}</pre>
                </details>
              </div>
            )}
