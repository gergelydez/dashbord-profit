            ...
            {showTranzitPanel && tranzitOrders.length > 0 && (() => {
              ...
              const handleExport = () => {
                ...
                const html = '<!DOCTYPE html><html lang="ro"><head>'
                  ...
                  + '  var labelUrl=rawUrl2.startsWith("http://")||rawUrl2.startsWith("https://")'
                  + '    ?(rawUrl2.includes("xconnector.app")?APP_ORIGIN+"/api/connector/label-proxy?url="+encodeURIComponent(rawUrl2):rawUrl2)'
                  + '    :APP_ORIGIN+rawUrl2;'
                  + '  fetch(labelUrl).then(function(r){'
                  + '    if(!r.ok)throw new Error("HTTP "+r.status);'
                  + '    return r.blob();'
