            ...
            {showTranzitPanel && tranzitOrders.length > 0 && (() => {
              ...
              const handleExport = () => {
                ...
                const html = '<!DOCTYPE html><html lang="ro"><head>'
                  ...
                  + '  var labelUrl=rawUrl.indexOf("xconnector.app")>=0?APP_ORIGIN+"/api/connector/label-proxy?url="+encodeURIComponent(rawUrl):rawUrl.indexOf("http")===0?rawUrl:APP_ORIGIN+rawUrl;'
                  ...
                  + '    document.getElementById("lm-bloburl").value=url;'
                  + '  }).catch(function(e){'
                  + '    document.getElementById("lm-status").textContent="Eroare: "+e.message;'
                  + '  });'
                  + '}'
                  + 'function fetchAndDownload(idx){'
                  + '  var o=ORDERS[idx];if(!o.awb)return;'
                  ...
                  + '  var labelUrl=rawUrl2.indexOf("xconnector.app")>=0?APP_ORIGIN+"/api/connector/label-proxy?url="+encodeURIComponent(rawUrl2):rawUrl2.indexOf("http")===0?rawUrl2:APP_ORIGIN+rawUrl2;'
                  + '  fetch(labelUrl).then(function(r){'
                  + '    if(!r.ok)throw new Error("HTTP "+r.status);'
                  + '    return r.blob();'
                  + '  }).then(function(blob){'
                  ...
                  + '    a.download="AWB_"+o.courier.toUpperCase()+"_"+o.awb+".pdf";'
                  + '    a.click();'
                  + '    if(btn){btn.textContent="✅ Descărcat";btn.style.background="#10b981";}'
                  + '    if(lmbtn){lmbtn.textContent="⬇ Download PDF";lmbtn.style.background="#f97316";}'
                  + '  }).catch(function(e){'
                  + '    if(btn){btn.textContent="⚠ Eroare";btn.style.background="#ef4444";}'
                  + '    if(lmbtn)lmbtn.textContent="⬇ Download PDF";'
                  + '    alert("Eroare la descărcare: "+e.message);'
                  + '  });'
                  + '}'
                  + 'function closeLabel(){document.getElementById("label-modal").style.display="none";var f=document.getElementById("lm-frame");f.src="";}'
                  + 'function closeModal(){document.getElementById("prod-modal").style.display="none";}'
                  + 'window.onload=function(){'
                  + '  ORDERS.forEach(function(o,idx){'
                  + '    if(packed[o.id]){'
                  + '      var card=document.getElementById("card-"+idx),row=document.getElementById("chk-"+idx);'
                  + '      var box=document.getElementById("box-"+idx),lbl=document.getElementById("lbl-"+idx),tim=document.getElementById("time-"+idx);'
                  + '      card.style.opacity=".55";card.style.transform="scale(.98)";'
                  + '      row.style.border="2px solid rgba(16,185,129,.35)";row.style.background="rgba(16,185,129,.06)";'
                  + '      box.innerHTML="✓";box.style.background="#10b981";box.style.border="2px solid #10b981";box.style.color="white";'
                  + '      lbl.textContent="PREGĂTIT ✅";lbl.style.color="#059669";'
                  + '      var t=new Date(packed[o.id]);tim.textContent=t.toLocaleTimeString("ro-RO",{hour:"2-digit",minute:"2-digit"});tim.style.display="";'
                  + '    }'
                  + '  });'
                  + '  updateProgress();'
                  + '};'
                ...
                const htmlFinal = html
                  .replace(/<script>/, '<script>var APP_ORIGIN='+JSON.stringify(appOrigin)+';')
